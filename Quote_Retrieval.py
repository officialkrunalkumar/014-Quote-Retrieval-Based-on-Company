...
def get_company_quotes(base_url, company_id, token):
    headers = {
        "Authorization": f"Bearer {token}",
        "Content-Type": "application/json"
    }
    association_url = f"{base_url}/crm/v4/objects/companies/{company_id}/associations/quotes"
...
def main(event):
  ...
  quoteIds = get_company_quotes(base_url, cId, token)
